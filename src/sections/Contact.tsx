...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      setFeedbackMessage(t.contact.messages.fillAllFields);
      return;
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(formData.email)) {
      setFeedbackMessage(t.contact.messages.invalidEmail);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        setFeedbackMessage(t.contact.messages.success);
        setFormData({ email: "", subject: "", message: "" });
      } else {
        setFeedbackMessage(t.contact.messages.error);
      }
    } catch (error) {
      setFeedbackMessage(t.contact.messages.error);
    } finally {
      ...
      >
        <div className="flex-1 space-y-8">
          <motion.h2
            ...
            custom={0}
          >
            {t.contact.title}<span className="text-[#6366f1]">.</span>
          </motion.h2>
          <motion.p
